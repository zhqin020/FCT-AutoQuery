...
# Defensive: ensure the standard-library `logging.handlers` submodule is available
# Some environments may shadow the stdlib `logging` module; if `handlers` is
# missing as an attribute on the imported `logging` module, attempt to import
# the handlers submodule explicitly and attach it so later references like
# `logging.handlers.RotatingFileHandler` work as expected.
try:
    # Access attribute to trigger AttributeError if missing
    _ = logging.handlers  # type: ignore[attr-defined]
except Exception:
    try:
        import importlib

        handlers_mod = importlib.import_module("logging.handlers")
        setattr(logging, "handlers", handlers_mod)
    except Exception:
        # Best-effort fallback; if this fails, later code will raise a clear ImportError
        pass
'''
Default logging configuration structure in json:
    "logging": {
        "level": "DEBUG",
...
def setup_logging(name: Optional[str] = None, skip_rotation: bool = False, is_subprocess: Optional[bool] = None, log_level: Optional[object] = None, log_file: Optional[str] = None) -> logging.Logger:
    ...
    with _initialization_lock:
        ...
        # Config values (allow overrides via parameters)
        if log_level is None:
            ...
            if isinstance(log_level, int):
                ...
                log_level_val = getattr(logging, str(log_level).upper(), logging.INFO)

        log_base = log_cfg.get("log_base", "fct-")
        # Allow callers to override the configured log_file template/path
        log_file_tmpl = log_file if log_file is not None else log_cfg.get("log_file", "<module.name>")
