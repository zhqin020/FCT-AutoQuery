...
*   **FR-04 结果状态判定**：
    ...
    *   **分支 D (页面访问异常)**：如果数据采集过程中，出现网络或数据异常，重试3次（可配置）则记录failed, 进行下一个case 进行采集。
*   **FR-05 模态框(Modal)管理**：
...
### 3.2 重复运行
*   **FR-30 支持重新运行**：
    *   程序采集后，记录case 采集状态，success, no_data, failed.  当程序运行时，可以对 failed 以及没有处理的继续采集。对已经采集成功和no data 的记录，进行跳过处理
...
*   **FR-09 文件备份 (JSON)**：
    *   每个案件生成一个独立的 JSON 文件。
    *   命名格式（参考文件）：`output/json/2021/IMM-1-21-20210101.json`。
...
CREATE TABLE docket_entries(
    ...
    PRIMARY KEY(id),
    CONSTRAINT docket_entries_court_file_no_fkey FOREIGN key(case_number) REFERENCES cases(case_number)
);
CREATE UNIQUE INDEX docket_entries_court_file_no_id_from_table_key ON public.docket_entries USING btree (case_number, id_from_table);
